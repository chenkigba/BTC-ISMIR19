...
class CNN(nn.Module):
    def __init__(self, config):
        ...
        self.conv_linear = nn.Conv2d(
            128, config["num_chords"], kernel_size=(1, 1), padding=0
        )
...
class CRNN(nn.Module):
    ...
    def forward(self, x, labels):
        ...
        h0 = torch.zeros(4, conv.size(0), self.hidden_size).to(
...
if __name__ == "__main__":
    ...
    crf = Crf(
        num_chords=config.model["num_chords"], timestep=config.model["timestep"]
    ).to(device)
