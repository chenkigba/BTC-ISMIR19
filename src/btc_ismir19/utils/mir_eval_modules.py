...
def audio_file_to_features(audio_file, config):
    ...
    while (
        len(original_wav)
        > currunt_sec_hz + config.mp3["song_hz"] * config.mp3["inst_len"]
    ):
...
def root_majmin_score_calculation(
    valid_dataset, config, mean, std, device, model, model_type, verbose=False
):
        ...
        try:
            ...
            with torch.no_grad():
                ...
                for t in range(num_instance):
                    if model_type == "btc":
                        encoder_output, _ = model.self_attn_layers(
                        ...
                        prediction, _, _, _ = model(
                            feature[:, n_timestep * t : n_timestep * (t + 1), :],
                            torch.randint(config.model["num_chords"], (n_timestep,)).to(
                                device
                            ),
                        )
    ...
    for m in root_majmin:
        metrics_.average_score[m] = np.sum(
            np.multiply(metrics_.score_list_dict[m], tmp)
        )

    return metrics_.score_list_dict, song_length_list, metrics_.average_score


def root_majmin_score_calculation_crf(
    valid_dataset,
    config,
    mean,
    std,
...
def large_voca_score_calculation(
    valid_dataset, config, mean, std, device, model, model_type, verbose=False
):
...
def large_voca_score_calculation_crf(
    valid_dataset,
    config,
    mean,
    std,
