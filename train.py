...
from btc_ismir19.utils.mir_eval_modules import (
    large_voca_score_calculation,
    root_majmin_score_calculation,
)
...
train_dataset2 = AudioDataset(
    config,
    root_dir=config.path["root_path"],
    dataset_names=(args.dataset2,),
    num_workers=20,
    preprocessing=False,
    train=True,
    kfold=args.kfold,
)
train_dataset3 = AudioDataset(
    config,
    root_dir=config.path["root_path"],
    dataset_names=(args.dataset3,),
    num_workers=20,
    preprocessing=False,
    train=True,
    kfold=args.kfold,
...
valid_dataloader = AudioDataLoader(
    dataset=valid_dataset, batch_size=config.experiment["batch_size"], drop_last=False
)
...
if os.path.isfile(os.path.join(asset_path, ckpt_path, ckpt_file_name % restore_epoch)):
    checkpoint = torch.load(
        os.path.join(asset_path, ckpt_path, ckpt_file_name % restore_epoch)
    )
...
z_path = os.path.join(
    ...
    "result",
    mp3_string + feature_string + "mix_kfold_" + str(args.kfold) + "_normalization.pt",
)
if os.path.exists(z_path):
    ...
    logger.info(
        "Global mean and std (training set, k fold index %d) calculation complete"
        % args.kfold
    )
...
for epoch in range(restore_epoch, config.experiment["max_epoch"]):
    ...
    result = {
        "loss/tr": np.mean(train_loss_list),
        "acc/tr": correct.item() / total,
        "top2/tr": (correct.item() + second_correct.item()) / total,
    }
    for tag, value in result.items():
        tf_logger.scalar_summary(tag, value, epoch + 1)
    logger.info(
        "training loss for %d epoch: %.4f" % (epoch + 1, np.mean(train_loss_list))
    )
    logger.info(
        "training accuracy for %d epoch: %.4f" % (epoch + 1, (correct.item() / total))
    )
    logger.info(
        "training top2 accuracy for %d epoch: %.4f"
        % (epoch + 1, ((correct.item() + second_correct.item()) / total))
    )
    ...
    with torch.no_grad():
        ...
        val_total = 0.0
        val_correct = 0.0
        val_second_correct = 0.0
        validation_loss = 0
        n = 0
        for i, data in enumerate(valid_dataloader):
            (
                val_features,
                val_input_percentages,
                val_chords,
                val_collapsed_chords,
                val_chord_lens,
                val_boundaries,
            ) = data
            ...
            val_prediction, val_loss, weights, val_second = model(
                val_features, val_chords
            )
        ...
        result = {
            ...
            "acc/val": val_correct.item() / val_total,
        ...
        logger.info(
            "validation accuracy(%d): %.4f"
            % (epoch + 1, (val_correct.item() / val_total))
        )
        logger.info(
            "validation top2 accuracy(%d): %.4f"
            % (
                epoch + 1,
                ((val_correct.item() + val_second_correct.item()) / val_total),
            )
        )
        ...
        if best_acc < val_correct.item() / val_total:
            ...
            logger.info(
                "==== best accuracy is %.4f and epoch is %d" % (best_acc, epoch + 1)
            )
            logger.info(
                "saving model, Epoch %d, step %d" % (epoch + 1, current_step + 1)
            )
            model_save_path = os.path.join(
                asset_path, "model", ckpt_file_name % (epoch + 1)
            )
            state_dict = {
    ...
    if args.early_stop and (early_stop_idx > 9):
...
# Load model
if os.path.isfile(
    os.path.join(asset_path, ckpt_path, ckpt_file_name % last_best_epoch)
):
    checkpoint = torch.load(
        os.path.join(asset_path, ckpt_path, ckpt_file_name % last_best_epoch)
    )
...
if args.voca:
    ...
    score_list_dict1, song_length_list1, average_score_dict1 = (
        large_voca_score_calculation(
    ...
    score_list_dict2, song_length_list2, average_score_dict2 = (
        large_voca_score_calculation(
    ...
    score_list_dict3, song_length_list3, average_score_dict3 = (
        large_voca_score_calculation(
            valid_dataset=valid_dataset3,
            config=config,
            model=model,
            model_type=args.model,
            mean=mean,
    ...
    for m in score_metrics:
        average_score = (
            np.sum(song_length_list1) * average_score_dict1[m]
            + np.sum(song_length_list2) * average_score_dict2[m]
            + np.sum(song_length_list3) * average_score_dict3[m]
        ) / (
            np.sum(song_length_list1)
            + np.sum(song_length_list2)
            + np.sum(song_length_list3)
        )
