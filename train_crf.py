...
if os.path.isfile(
    os.path.join(asset_path, ckpt_path, load_ckpt_file_name % load_restore_epoch)
):
...
crf = Crf(num_chords=config.model["num_chords"], timestep=config.model["timestep"]).to(
    device
)
optimizer = optim.Adam(
    filter(lambda p: p.requires_grad, crf.parameters()),
    lr=0.01,
...
for epoch in range(restore_epoch, config.experiment["max_epoch"]):
    ...
    if args.early_stop and (early_stop_idx > 5):
